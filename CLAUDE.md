...
**Wave 4 open text (Q14 "one thing helping, or one thing getting in the way") classifier:** `src/data/helpHinder.js`, deterministic and credit-free. Every answer lands in exactly one of helping / in the way / both / no answer; there is NO "not classified" bucket (Mike, 2026-09-14). It scores clauses with weighted cue lists, halves concessions ("X but Y" argues Y; a leading "while…" clause), flips negations ("I wouldn't say anything is in the way" = helping; "not seeing how it can help" = in the way), drops wishes ("would love to…", "before we can…"), and decides cue-less answers by shape (effort-led "Finding time to…" = in the way; a bare noun phrase = helping). "Both" needs two full-weight one-sided sentences or explicit "Helping: … In the way: …" leads. Labeled regression net: `node scripts/check-classifier.mjs [--s4-url=<csv>] [--verbose]` (Wave 3's struggle answers must read "in the way", its excitement answers "helping", floor 90% each, plus hand-labeled Wave 4 answers; 90.8% / 99.0% on 2026-09-14). To see why an answer landed where it did: `explainHelpHinder(text)` from that module prints the segments and scores. When Mike flags a wrong bucket: add the answer to `HAND` in check-classifier.mjs with the right label first, then tune a cue, then rerun; never special-case one answer.

**Regression net (no credits):** `node scripts/check-classifier.mjs` (open-text classifier, above), `node scripts/check-data.mjs` (data layer, S1 local + S2/S3 live + S4 sample; `--no-s4` for the 3-wave path; `--s4-url=<csv url>` once the real sheet exists) and `node scripts/smoke-ui.mjs` (headless Puppeteer, needs `npx vite --port 5010 --strictPort` running; clicks every tab in both modes, fails on any console error beyond the known framer-motion SVG baseline). Run both before pushing.
